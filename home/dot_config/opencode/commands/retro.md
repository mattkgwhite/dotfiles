...
Identify approaches, tools, or decisions that worked well. For each one, load the `memory` skill and either confirm it is already covered by an existing rule or strengthen/add a rule that encodes the good practice. Report back a 1-sentence summary per point.

## 2. What went wrong

Identify mistakes, incorrect assumptions, wasted steps, or anything that required correction. For each one, load the `memory` skill and add a new rule or tighten an existing rule so the same mistake does not recur in future sessions. Report back a 1-sentence summary per point.

## 3. Output

After applying all lessons via the `memory` skill, provide a compact summary:
